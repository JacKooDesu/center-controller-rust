...
pub async fn run() {
    ...
    if let Ok(socket) = socket_result {
        ...
        handler.socket = Some(arc_socket.clone());
            ...
            loop {
                match arc_socket.recv_from(&mut buf).await {
                    Ok((len, addr)) => {
                        ...
                        let client = clients.entry(addr).or_insert(ClientStatus::new(addr));
...
pub async fn stop() {
    ...
    if let Some(task) = handler.task.take() {
        task.abort();
    }
    handler.socket = None;
...
async fn decode_jpeg_packet(addr: SocketAddr, header: JPEGHeader, data: &Vec<u8>) {
    ...
    let decoder = decoders.entry(addr).or_insert_with(||JPEGDecoder::new(header));
